...
<div class="main-panel">
    <div class="heading-section">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h4 class="heading-title">Edit Resources Guide</h4>
            </div>
    ...
    <div class="di-section">
        <div class="add-form-info">
            <form class="" method="post" enctype="multipart/form-data" id="guide-form">
            @csrf
                ...
                <div class="filter-info">
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="plan" class="form-control">
                                    @foreach(getStaticSubscription() as $key=>$value)
                                        <option value="{{$key}}" @if($key==$guide->subscription_type) selected @endif >{{$value}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Title</label>
                                <input type="text" name="title" class="form-control {{$errors->first('title')?'is-invalid':''}}" placeholder="Title" value="{{$guide->title}}" />
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea name="description" class="form-control {{$errors->first('description')?'is-invalid':''}}" placeholder="Description" id="guide-description" >{!! $guide->description !!}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control {{$errors->first('status')?'is-invalid':''}}">
                                    ...
                                    @foreach(status_array() as $key=>$row)
                                        <option value="{{$key}}" @if($guide->status==$key) selected @endif >{{$row}}</option>
                                    @endforeach
                                </select>
...
<script>
    $(document).ready(function(){
        ...
        /*-----------------------------------------------------------------------------------------------*/
        if($("#guide-description").length){
            CKEDITOR.replace( 'guide-description' );
        }
        /*-----------------------------------------------------------------------------------------------*/        
