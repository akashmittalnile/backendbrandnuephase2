...
<div class="main-panel">
    <div class="heading-section">
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                <h4 class="heading-title">Edit Instructional Template</h4>
            </div>
                ...
                <div class="filter-info">
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="plan" class="form-control">
                                    @foreach(getStaticSubscription() as $key=>$value)
                                        <option value="{{$key}}" @if($key==$template->subscription_type) selected @endif >{{$value}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Title</label>
                                <input type="text" name="title" class="form-control {{$errors->first('title')?'is-invalid':''}}" placeholder="Title" value="{{$template->title}}" />
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea name="description" class="form-control {{$errors->first('description')?'is-invalid':''}}" placeholder="Description" id="template-description">{!! $template->description !!}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <select name="status" class="form-control {{$errors->first('status')?'is-invalid':''}}">
                                    ...
                                    @foreach(status_array() as $key=>$row)
                                        <option value="{{$key}}" @if($template->status==$key) selected @endif >{{$row}}</option>
                                    @endforeach
                                </select>
